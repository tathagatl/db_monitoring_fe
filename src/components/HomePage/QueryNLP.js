...
function QueryNLP() {
    ...
    const runQuery = async () => {
        const response = await axios.post('http://localhost:5000/querynlp', {
            query
        })
        setOutput(response.data['response']['query_output'])
    }
    return (
        <Box sx={{
            display: 'flex',
            ...
            <Container sx={{ mt: '1rem' }}>
                <Grid container columns={12}>
                    <Grid item sm={12} md={4} sx={{
                        ...
                        <TextField id="outlined-basic" value={query} onChange={e => setQuery(e.target.value)} label="Natural Language Query" variant="outlined" sx={{ mb: '0.5rem' }} />
